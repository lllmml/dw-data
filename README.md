...
E2.3-A 运行方法见 [recovery evidence guide](docs/guides/nanjing_topology_recovery.md)。E2.3-D Design Review 已通过；D1 synthetic completion contract/cohort refinement 已完成，运行与校验见 [D1 guide](docs/guides/nanjing_synthetic_completion_contract.md)。D2 synthetic topology proposal-only 已实现，见 [D2 guide](docs/guides/nanjing_synthetic_topology_proposals.md)；业务输入保持未确认，提案与 accepted topology 分离。D2 提案仍未被 apply。另经本次 D3 授权，已实现窄范围 deterministic recovery 与独立 accepted topology v2，见 [D3 guide](docs/guides/nanjing_deterministic_topology_recovery.md) 和 [D3 handoff](docs/handoff/2026-09-17-e2-3-d3-deterministic-recovery.md)。D4 另经授权实现 source-constrained synthetic backbone proposal-only，见 [D4 guide](docs/guides/nanjing_synthetic_backbone.md) 和 [D4 contract](docs/spec/nanjing_synthetic_backbone.md)。D4.1 进一步对 137 条“有 source Line、无 accepted Line component”的 Feeder 建立 source placement evidence，并在唯一证据下生成 proposal-only engineering anchor 与 Line representation，再重放 D4 backbone；见 [D4.1 guide](docs/guides/nanjing_placement_evidence.md) 和 [D4.1 contract](docs/spec/nanjing_placement_evidence.md)。停止等待 D4.1 交付 review；accepted v2 不变，D2/D4/D4.1 提案均未 APPROVE/APPLY，不整体接受 S2，不进入 no-source-Line free layout、E3 或 OpenDSS。v1 completion export 已完成：completion ledger、source-shaped delivery、per-row provenance、manifest/report、确定性 archive 与独立 validator，一条命令产出 `nanjing-derived-v1.zip`，两两运行字节一致；见 [v1 export guide](docs/guides/nanjing_v1_completion_export.md) 和 [v1 contract](docs/spec/nanjing_v1_completion_export.md)。交付内含 PROPOSED 行且未经 APPROVE，不进入 E3 或 OpenDSS。
